...
check_env() {
	if [ -z "$PROGRAM_KEYPAIR" ]; then
		echo "PROGRAM_KEYPAR env variable is not set" >&2
		exit 1
	fi
	if [ -z "$TYPE" ]; then
...
main() {
	...
	ID="$(echo -n "$PROGRAM_KEYPAIR" | solana address -k -)"
	...
	mv target/verifiable/polymer_prover.so "target/verifiable/polymer_prover.${TYPE}.so"
